...
class MNISTDataset(Dataset):
    ...
    def __init__(self, root_dir, split, augment_data=False, model_type='cnn', args=None,
                 transform=None, eval_transform=None):
        self.root = root_dir
        ...
        try:
            logging.info('Original subclass counts:')
            logging.info(np.bincount(original_labels))
        except NameError:
            pass

        # Subsample 8 subclass
        if split == "train":
            try:
                logging.info('Subsampling subclasses: {8: 0.05}')
            except NameError:
                pass
            data, original_labels = self.subsample_digits(data, original_labels, {8: 0.05})
            try:
                logging.info('New subclass counts:')
                logging.info(np.bincount(original_labels))
            except NameError:
                pass
        ...
        # For transform and eval_transform, we will always specify in the dataloader function
        if transform is None:
            resize = False  # Set to true if your model expects input images to be 32x32 rather than 28x28
            self.train_transform = get_transform_MNIST(resize=resize, augment=augment_data)
        else:
            self.train_transform = transform
            
        if eval_transform is None: 
            resize = False  # Set to true if your model expects input images to be 32x32 rather than 28x28
            self.eval_transform = get_transform_MNIST(resize=resize, augment=False)
        else:
...
def load_umnist(args, train_shuffle=True, 
                transform=None, eval_transform=None):
    """
    ...
    Returns:
    - (train_loader, val_loader, test_loader): Tuple of dataloaders for train and test
    """
    # augment_data not used
    umnist_train = MNISTDataset(root_dir=args.root_dir, split='train', augment_data=True, args=args,
                                transform=transform, eval_transform=eval_transform)

    umnist_val = MNISTDataset(root_dir=args.root_dir, split='val', augment_data=False, args=args,
                              transform=transform, eval_transform=eval_transform)

    umnist_test = MNISTDataset(root_dir=args.root_dir, split='test', augment_data=False, args=args,
                               transform=transform, eval_transform=eval_transform)

    train_loader = DataLoader(umnist_train, batch_size=args.bs_trn,
                              shuffle=train_shuffle,
                              num_workers=args.num_workers)
    val_loader   = DataLoader(umnist_val, batch_size=args.bs_val,
                              shuffle=False, num_workers=args.num_workers)
    test_loader  = DataLoader(umnist_test, batch_size=args.bs_val,
                              shuffle=False, num_workers=args.num_workers)
